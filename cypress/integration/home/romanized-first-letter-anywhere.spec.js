import { SEARCH_TYPES } from '../../../src/js/constants';

describe('Home page Search tests', () => {
  const searchString = 'jmtatssd';

  describe('Romanized First Letter Search anywhere(E)', () => {
    beforeEach(() => {
      cy.visit('/');

      cy.get('#search-type')
        .select(SEARCH_TYPES.ROMANIZED_FIRST_LETTERS_ANYWHERE);
    });

    it(`should return search results for ${searchString} in drop-down`, () => {
      cy.get('#search')
        .type(searchString);

      cy.get('.search-result span')
        .should('contain', 'jo')
        .should('contain', 'mwgih')
        .should('contain', 'Twkur')
        .should('contain', 'Apuny')
        .should('contain', 'qy')
        .should('contain', 'soeI')
        .should('contain', 'dyvY')

    });

    it(`should contain searchedString in first letters of the words [in results]`, () => {
      cy.get('.search-highlight-word').as('result')

      let firstLetterSearch = ''
      cy.get('@result').forEach(result => {
        firstLetterSearch += result.innerHtml[0];
      });
      const isMatched = firstLetterSearch
      expect(first)
      console.log(firstLetterSearch);
    })
  })